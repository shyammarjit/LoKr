...
class LoRAAttnProcessor2_0(nn.Module):
    ...
    def __init__(self, hidden_size, cross_attention_dim=None, network_alpha=None, out_rank=None, factor=-1, lora_rank=4, **kwargs):
        super().__init__()
        if not hasattr(F, "scaled_dot_product_attention"):
            raise ImportError("AttnProcessor2_0 requires PyTorch 2.0, to use it, please upgrade PyTorch to 2.0.")
        ...
        self.to_q_lora = LoKr(q_hidden_size, q_hidden_size, network_alpha, factor=factor, lora_rank=lora_rank)
        self.to_k_lora = LoKr(cross_attention_dim or hidden_size, hidden_size, network_alpha, factor=factor, lora_rank=lora_rank)
        self.to_v_lora = LoKr(cross_attention_dim or v_hidden_size, v_hidden_size, network_alpha, factor=factor, lora_rank=lora_rank)
        self.to_out_lora = LoKr(out_hidden_size, out_hidden_size, network_alpha, factor=factor, lora_rank=lora_rank)
    ...
    def __call__(self, attn: Attention, hidden_states, encoder_hidden_states=None, attention_mask=None, scale=1.0):
        ...
        query = attn.to_q(hidden_states) + scale * self.to_q_lora(hidden_states) # Q
        ...
        key = attn.to_k(encoder_hidden_states) + scale * self.to_k_lora(encoder_hidden_states) # K

        value = attn.to_v(encoder_hidden_states) + scale * self.to_v_lora(encoder_hidden_states) # V
        ...
        # linear proj
        hidden_states = attn.to_out[0](hidden_states) + scale * self.to_out_lora(hidden_states) # out
